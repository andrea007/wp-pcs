...
function eventerra_wpb_init() {
  ...
  $params->update($param);
	...
	$css_animation = array(
		'type' => 'dropdown',
		'heading' => esc_html__( 'CSS Animation', 'eventerra' ),
		'param_name' => 'css_animation',
		'admin_label' => true,
		'value' => array(
			esc_html__( 'No', 'eventerra' ) => '',
			esc_html__( 'Top to bottom', 'eventerra' ) => 'top-to-bottom',
			esc_html__( 'Bottom to top', 'eventerra' ) => 'bottom-to-top',
			esc_html__( 'Left to right', 'eventerra' ) => 'left-to-right',
			esc_html__( 'Right to left', 'eventerra' ) => 'right-to-left',
			esc_html__( 'Appear from center', 'eventerra' ) => 'appear',
			/***/			
		...
		'description' => esc_html__( 'Select type of animation if you want this element to be animated when it enters into the browsers viewport. Note: Works only in modern browsers.', 'eventerra' )
	);
	$css_animation_delay = array(
		'type' => 'textfield',
		'heading' => esc_html__( 'Delay before animation starts (milliseconds)', 'eventerra' ),
	...
	$css_animation_custom=$css_animation;
	$css_animation_custom['value']=array_merge(array_slice($css_animation_custom['value'], 0, 1, true), array(esc_html__( 'Custom animation for this element', 'eventerra' ) => 'custom',), array_slice($css_animation_custom['value'], 1, count($css_animation_custom['value'])-1, true));
	...
	vc_add_param('vc_custom_heading', $css_animation_custom);
