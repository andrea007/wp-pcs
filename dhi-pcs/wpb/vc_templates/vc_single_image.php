...
if(get_option("eventerra_lazyload") == 'true') {
	$img['thumbnail']=eventerra_img_insert_lazyload($img['thumbnail']);
}
...
if ( $link ) {
	if  ($onclick == 'img_link_large' || $onclick == 'link_image') {
		$html = eventerra_hover_extras($img['thumbnail'], $link, false, 'target="' . $img_link_target . '"');
	} else {
		$html = eventerra_hover_extras($img['thumbnail'], false, $link, false, 'target="' . $img_link_target . '"');
	}	
} else {
	$html = $img['thumbnail'];
}
...
if($remove_margins == 'yes') {
	$class_to_filter.=' wpb_single_image-remove-margins';
}
