...
$color = $accent_color = $el_class = $el_id = $layout = $css =
...
$i_custom_background_color = $i_size = $i_css_animation = $css_animation = '';
...
$class .= ($title_size!='') ? ' vc_title_size_'.$title_size : '';
...
if('custom' === $border_color && '' !== $border_custom_color) {
	$inline_css = 'style="' . vc_get_css_color( 'border-color', $border_custom_color ) . '"';
} elseif('custom' === $color && '' !== $accent_color) {
...
if('custom' == $color) {
	if($accent_color!='')
		$inline_css_h=' style="'.vc_get_css_color('color', $accent_color).'"';
} elseif('default' != $color) {
	if($tmp=eventerra_wpb_get_std_color_code($color))
		$inline_css_h=' style="color:'.$tmp.'"';
}
...
<div class="$css_class"$inline_css_h $wrapper_attributes_html><span
