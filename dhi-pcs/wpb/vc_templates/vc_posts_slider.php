...
$orderby = $order = $el_class = $el_id = $link_image_start = '';
...
while ( $my_query->have_posts() ) {
	...
	// if ( $thumbnail == '' ) $thumbnail = esc_html__("No Featured image set.", "eventerra");

	// Link logic
	if ( $link != 'link_no' ) {
		if ( $link == 'link_post' ) {
			$link_image_start = '<a class="link_image" href="' . esc_url(get_permalink( $post_id )) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'eventerra' ), the_title_attribute( 'echo=0' ) ) . '">';
...
if ( $teasers ) {
	...
	$teasers = esc_html__( "Nothing found.", "eventerra" );
}
...
$output .= "\n\t" . '<div class="' . $css_class . '" ' . implode( ' ', $wrapper_attributes ) . '>';
