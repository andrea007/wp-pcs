...
function wie_upload_import_file() {
	...
	if ( ! empty( $_POST ) && ! empty( $_FILES['wie_import_file'] ) && check_admin_referer( 'wie_import', 'wie_import_nonce' ) ) { // check_admin_referer prints fail page and dies
		...
		if ( 'wie' != $wp_filetype['ext'] && ! wp_match_mime_types( 'wie', $wp_filetype['type'] ) ) {
			wp_die(
				wp_kses(
					__( 'You must upload a <b>.wie</b> file generated by this plugin.', 'om-theme-framework' ),
					array(
						'b' => array()
					)
				),
...
function wie_process_import_file( $file ) {
	...
	if ( ! file_exists( $file ) ) {
		wp_die(
			esc_html__( 'Import file could not be found. Please try again.', 'om-theme-framework' ),
			'',
			array( 'back_link' => true )
		);
	}
...
function wie_import_data( $data ) {
	...
	if ( empty( $data ) || ! is_object( $data ) ) {
		wp_die(
			esc_html__( 'Import data could not be read. Please try a different file.', 'om-theme-framework' ),
			'',
			array( 'back_link' => true )
		);
	}
	...
	foreach ( $data as $sidebar_id => $widgets ) {
		...
		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
			...
			$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'om-theme-framework' );
		}
		...
		foreach ( $widgets as $widget_instance_id => $widget ) {
			...
			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
				...
				$widget_message = esc_html__( 'Site does not support widget', 'om-theme-framework' ); // explain why widget not imported
			}
			...
			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
				...
				foreach ( $single_widget_instances as $check_id => $check_widget ) {
					...
					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
						...
						$widget_message = esc_html__( 'Widget already exists', 'om-theme-framework' ); // explain why widget not imported
			...
			if ( ! $fail ) {
				...
				if ( $sidebar_available ) {
					...
					$widget_message = esc_html__( 'Imported', 'om-theme-framework' );
				} else {
					$widget_message_type = 'warning';
					$widget_message = esc_html__( 'Imported to Inactive', 'om-theme-framework' );
				}
			...
			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = ! empty( $widget['title'] ) ? $widget['title'] : esc_html__( 'No Title', 'om-theme-framework' ); // show "No Title" if widget instance is untitled
