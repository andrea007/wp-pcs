...
class Envato_Protected_API {
  ...
  public function __construct( $user_name = '', $api_key = '' ) {
    ...
    if ( $user_name == '' ) {
      $this->set_error( 'user_name', esc_html__( 'Please enter your Envato Marketplace Username.', 'om-theme-framework' ) );
    }
      
    if ( $api_key == '' ) {
      $this->set_error( 'api_key', esc_html__( 'Please enter your Envato Marketplace API Key.', 'om-theme-framework' ) );
    }
  ...
  public function private_user_data( $set = '', $user_name = '', $set_data = '', $allow_cache = false, $timeout = 300 ) { 
    ...
    if ( $set == '' ) {
      $this->set_error( 'set', esc_html__( 'The API "set" is a required parameter.', 'om-theme-framework' ) );
    }
  ...
  public function wp_download( $item_id ) {
    ...
    if ( ! isset( $item_id ) ) {
      $this->set_error( 'item_id', esc_html__( 'The Envato Marketplace "item ID" is a required parameter.', 'om-theme-framework' ) );
    }
  ...
  protected function remote_request( $url ) {
    ...
    if ( is_wp_error( $request ) ) {
    	echo wp_strip_all_tags($request->get_error_message());
    	return false;
    }
